...
以上能力在 **个股分析页** 集中呈现：左侧缠论信号与走势判断，中间多级别 K 线，右侧 AI 策略建议、笔记与诊股悬浮球。PC 三栏布局信息密度高；移动端则保留 K 线核心，详情通过底部抽屉展开。
...
> 重新生成 README 截图：先启动前后端开发服务，再执行 `cd scripts && npm install && npx playwright install chromium && npm run screenshots`。
