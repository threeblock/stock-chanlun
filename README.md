...
下表汇总了各核心页面的实际界面，便于快速了解产品能力。截图由 `scripts/capture-readme-screenshots.mjs` 自动生成。

| 页面 | 平台 | 说明 | 截图 |
|------|------|------|------|
| 首页 | PC | 六大指数、涨跌家数、热门板块与人气股票，每 5 分钟自动刷新 | ![PC 首页](assets/screenshots/pc-home.png) |
| 个股分析 | PC | 三栏布局：左侧缠论信号 + 中间多级别 K 线 + 右侧 AI 策略与笔记；右下角悬浮球可展开 AI 诊股 | ![PC 个股](assets/screenshots/pc-stock.png) |
| 自选股 | PC | 添加/删除自选、实时涨跌幅、最后更新时间，每 2 分钟自动刷新 | ![PC 自选](assets/screenshots/pc-watchlist.png) |
| 条件选股 | PC | MACD+SKDJ 共振、缠论买卖点、基本面多维筛选，SSE 流式返回 | ![PC 选股](assets/screenshots/pc-screen.png) |
| 板块详情 | PC | 行业/概念成分股排行，支持一键加入自选 | ![PC 板块](assets/screenshots/pc-sector.png) |
| 首页 | 移动端 | 搜索栏 + 指数 + 涨跌家数 + 热门板块/股票，底部 Tab 导航 | ![移动首页](assets/screenshots/mobile-home.png) |
| 个股分析 | 移动端 | K 线全屏展示、级别切换、指标选择，详情通过底部抽屉展开 | ![移动个股](assets/screenshots/mobile-stock.png) |
...
打开应用，首屏即可纵览 A 股市场：主要指数、涨跌家数、热门板块与人气股票集中展示，每 5 分钟自动刷新。

![PC 首页 — 大盘概览与市场热点](assets/screenshots/pc-home.png)
...
同一套后端，PC 与移动端共用路由与 API。移动端针对触屏做了搜索、K 线滑动与底部 Tab 导航优化，外出也能快速查行情、看缠论。

![移动端首页](assets/screenshots/mobile-home.png)
...
规则策略引擎 + MACD 背驰检测 + 走势分类 + 多级别共振（日线+30分钟）+ LLM 自然语言分析（需配置 API Key，默认 DeepSeek V4 Pro）。

以上能力在 **个股分析页** 集中呈现：左侧缠论信号与走势判断，中间多级别 K 线（支持 MA/MACD/RSI/SKDJ 等指标切换与图表降采样），右侧 AI 策略建议、笔记与诊股悬浮球。PC 三栏布局信息密度高；移动端则保留 K 线核心，详情通过底部抽屉展开。

![PC 个股分析 — 缠论结构、多指标与 AI 策略](assets/screenshots/pc-stock.png)

![移动端个股 — K 线 + 级别切换 + 底部抽屉](assets/screenshots/mobile-stock.png)

### 6. 大盘概览

实时展示上证/深证/创业板/科创50/沪深300/中证500 指数、涨跌家数，行业板块涨跌排行。每 5 分钟自动刷新。（见上文 [项目概览](#项目概览) 首页截图。）

### 7. 行业/概念板块

支持查看板块涨跌排行、板块详情（成分股列表），PC 端和移动端均支持。点击首页板块卡片进入详情，成分股按涨跌幅排序，可一键加入自选。

![板块详情 — 半导体成分股](assets/screenshots/pc-sector.png)

### 8. 智能选股

MACD+SKDJ 双金叉共振 + 缠论买卖点过滤 + 基础指标过滤（涨跌幅、成交量、市盈率、市净率、行业）。SSE 流式返回，边算边展示，支持分页加载。

![条件选股 — 多维筛选与 SSE 流式结果](assets/screenshots/pc-screen.png)

### 9. 自选股监控

添加/删除自选、按自选时间排序，每 2 分钟自动刷新行情，顶部显示最后更新时间。

![自选股监控](assets/screenshots/pc-watchlist.png)

### 10. 个股扩展信息

五档盘口（买一到买五/卖一到卖五）、行业/题材概念、个股财经新闻，一站式聚合展示，减少前端请求次数。（见个股分析页「基本资料」区域。）
...
PC 个股页通过右下角 **悬浮球** 打开诊股面板；移动端仍在详情抽屉中使用对话区。AI 助手「缠师」支持流式对话：
...
# Vite 将 /api 与 /stock-chanlun/api 代理到 http://127.0.0.1:8010（可用 VITE_API_PROXY_TARGET 修改）
```
...
大盘指数 + 热门板块 + 热门股票 + 财经要闻，自动 5 分钟刷新。对应截图见 [项目概览](#项目概览)。

#### `/stock/:code` — 个股分析

三栏布局（左侧行情统计 + 中间 K 线图 + 右侧 AI 策略），支持键盘快捷键：R 刷新、1/5/D/W/M 切换级别。对应截图见 [功能特性 · AI 策略增强](#5-ai-策略增强)。

#### `/watchlist` — 自选股

添加/删除/刷新，含最后更新时间，自动 2 分钟刷新。对应截图见 [功能特性 · 自选股监控](#9-自选股监控)。

#### `/screen` — 条件选股

SSE 流式返回结果，含进度条和错误处理。对应截图见 [功能特性 · 智能选股](#8-智能选股)。

#### `/sector/:name` — 板块详情

成分股按涨跌幅降序排列。对应截图见 [功能特性 · 行业/概念板块](#7-行业概念板块)。

### 移动端

#### `/m/` — 移动首页

搜索栏 + 指数 + 热门板块 + 热门股票。对应截图见 [项目概览](#项目概览)。

#### `/m/stock/:code` — 移动个股

顶部价格 + K 线图 + 级别切换 + 信号摘要（可展开）+ 底部抽屉。对应截图见 [功能特性 · AI 策略增强](#5-ai-策略增强)。

#### `/m/watchlist`、`/m/screen`、`/m/sector/:name`

与 PC 端功能一致，布局适配触屏与窄屏。

> 重新生成 README 截图：先启动前后端开发服务，再执行：
>
> ```bash
> cd scripts && npm install && npx playwright install chromium && npm run screenshots
> ```
>
> 若 Vite 端口非 5173，请指定实际地址，例如：
> `SCREENSHOT_BASE_URL=http://localhost:5175/stock-chanlun npm run screenshots`
